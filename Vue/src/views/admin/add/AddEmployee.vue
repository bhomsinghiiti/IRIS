        ...
        <v-card class="custom-card" elevation="0">
            <v-card-text>
                <v-form @submit.prevent="submitForm">
                    <v-col>
                        <v-text-field variant="solo" placeholder="Employee full name" v-model="name"
                            bg-color="#DDE6ED"></v-text-field>
                    </v-col>
                    <v-col>
                        <v-text-field variant="solo" placeholder="Username" v-model="username"
                            bg-color="#DDE6ED"></v-text-field>
                    </v-col>
                    <v-col>
...
export default {
    ...
    methods: {
        async submitForm() {
            try {
                await axios.post('http://localhost:8080/admin/employee', {
                ...
                console.error(error);
                this.showErrorSnackbar('An error occurred while adding the employee.');
            }
        },
