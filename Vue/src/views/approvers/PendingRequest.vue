  ...
  <v-card width="100%" elevation="0">
    ...
    <div v-if="val === 2">
      <v-dialog v-model="show" max-width="600px" color="black">
          ...
          <v-toolbar style="background-color: #0ae489; border-radius: 20px 20px 0px 0px; text-align: center;">
            <v-toolbar-title style="font-size:x-large;">Varify Request</v-toolbar-title>
            <v-btn icon @click="closeDialog">
              <v-icon>mdi-close</v-icon>
            </v-btn>
          </v-toolbar>
...
import VarifyRequest from '@/views/approvers/VarifyRequest.vue';
...
export default {
  ...
  async created() {
    try {
      ...
      const response = await axios.get('http://localhost:8080/approver/requests/pending');
